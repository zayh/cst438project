...
class DatabaseObject:
  ...
  def __init__(self, data=None):
    ...
    if data is not None:
      for key in data:
        self.data['table_data'][key] = data[key]  
        
  def toJSON(self):
    ''' Returns a JSON string of the object. '''
    ''' Test with test_JSON '''
    output_data = { }
    for key in self.data['table_data']:
      if isinstance(self.data['table_data'][key],datetime.date):
        output_data[key] = self.data['table_data'][key].isoformat()
      else:
        output_data[key] = self.data['table_data'][key]
        
    jsonStr = json.dumps(output_data)
    return jsonStr
  ...
  def getColumns(self):
    ...
    returnVal = ''
    if 'table_layout' in self.data:
      returnVal = self.data['table_layout']
    return returnVal
      
  def getItem(self, column):
    ''' Accessor for most data '''
    returnVal = ''
    if column in self.data['table_data']:
      returnVal = self.data['table_data'][column]
    return returnVal

  def getUnique(self):
    ''' Returns a dict of columns that when combined, form a unique index '''
    returnVal = ''
    if 'unique_combos' in self.data:
      returnVal = self.data['unique_combos']
    return returnVal
  ...
  def setItem(self, column, value):
    ...
    success = False
    if (1):
      self.data['table_data'][column] = value
      success = True
    return success
  ...
  def getRow(self, value):
    ...
    table_layout = self.getColumns() # Dict of the table_layout
  ...
  def addToDatabase(self):
    ...
    table_layout = self.getColumns() # The layout is needed
  ...
  def saveToDatabase(self):
    ...
    table_layout = self.getColumns() # Need the column names
  ...
  def loadRow(self, row=None):
    ''' Takes a row tuple as input and stuffs it into the current object '''
    success = False
    table_layout = self.getColumns()
    if row is not None:
      for key in table_layout:          
